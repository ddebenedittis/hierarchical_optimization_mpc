...
class TasksCreatorHOMPCMultiRobot:
    ...
    def get_task_input_smooth(self):
        task_input_smooth = [
            ca.vertcat(self.u[0], -self.u[0], self.u[1], -self.u[1]),
            ca.vertcat(self.u[0], -self.u[0], self.u[1], -self.u[1]),
        ]
    ...
    def get_task_obs_avoidance(self, obstacle_pos: np.ndarray, threshold: float = 1.0):
        ...
        return [
            ca.vertcat(
                -((self.s[0] - (obstacle_pos[0])) ** 2) + threshold**2,
                -((self.s[1] - (obstacle_pos[1])) ** 2) + threshold**2,
            ),
            ca.vertcat(
                -((self.s[0] - obstacle_pos[0]) ** 2) + threshold**2,
                -((self.s[1] - obstacle_pos[1]) ** 2) + threshold**2,
    ...
    def get_task_formation(self, ref=None):
        ...
        task_formation_coeff = [TaskBiCoeff(1, 0, 1, 1, 0, 2**2)]
