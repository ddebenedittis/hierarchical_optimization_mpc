...
class Node:
    ...
    def __init__(
        ...
        if self.n_robots != self.degree + 1:
            raise ValueError(
                f'Number of robots {self.n_robots} does not match the degree {self.degree + 1} of the node {self.node_id}.'
            )
        ...
        self.alpha = np.concatenate(
            (
                st.step_size * np.ones((self.n_xi_o * self.n_robots_models.omni)),
        ...
        self.y_j = np.zeros(
            (2, self.n_priority, self.n_xi_o * (self.degree))
    ...
    def MPC(self) -> None:
        ...
        if self.model == 'omniwheel':
            if self.node_id == 0:
                self.s = RobCont(
                    omni=[np.array([-2, 2]) for _ in range(self.n_robots)],
                )
            elif self.node_id == 1:
                self.s = RobCont(omni=[np.array([2, 2]) for _ in range(self.n_robots)])
            elif self.node_id == 2:
                self.s = RobCont(omni=[np.array([2, -2]) for _ in range(self.n_robots)])
            elif self.node_id == 3:
                self.s = RobCont(omni=[np.array([-2, -2]) for _ in range(self.n_robots)])
            ...
            if self.node_id == 0:
                self.s = RobCont(
                    omni=[np.array([-2, 2, 0]) for _ in range(self.n_robots)],
                )
            elif self.node_id == 1:
                self.s = RobCont(omni=[np.array([2, 2, 0]) for _ in range(self.n_robots)])
            elif self.node_id == 2:
                self.s = RobCont(omni=[np.array([2, -2, 0]) for _ in range(self.n_robots)])
            elif self.node_id == 3:
                self.s = RobCont(omni=[np.array([-2, -2, 0]) for _ in range(self.n_robots)])
    ...
    def dual_update(self):
        ...
        self.rho_i[0, :, :] += self.alpha * (
            np.tile(self.y_i[:, 0 : self.n_x], self.degree) - self.y_j[0, :, :]
        )
    ...
    def create_connection(
        ...
        if added_robot > 0:
            ...
            self.y_i = np.pad(
                self.y_i,
                ((0, 0), (0, n_x * (self.n_robots) - self.y_i.shape[1])),
                mode='constant',
                constant_values=0,
            )
            self.rho_i = np.pad(
                self.rho_i,
                ((0, 0), (0, 0), (0, n_x * (self.degree) - self.rho_i.shape[2])),
                mode='constant',
                constant_values=0,
            )
            self.rho_j = np.pad(
                self.rho_j,
                ((0, 0), (0, 0), (0, n_x * (self.degree) - self.rho_j.shape[2])),
                mode='constant',
                constant_values=0,
            )
            self.y_j = np.pad(
                self.y_j,
                ((0, 0), (0, 0), (0, n_x * (self.degree) - self.y_j.shape[2])),
                mode='constant',
                constant_values=0,
            )
            ...
            self.s.uni.append(state_meas)
            self.s_init.uni.append(state_meas)
    ...
    def remove_connection(self, adjacency_vector: np.array, neigh_tasks: str, neigh_id: int):
        ...
        self.s.uni.pop(id_to_remove)
        self.s_init.uni.pop(id_to_remove)
