...
import distributed_ho_mpc.scenarios.agent_chain.settings as st
...
class Node:
    ...
    def __init__(
        ...
        self.receiver = MessageReceiver(
            self.node_id, self.neigh, self.y_j, self.rho_j, self.n_xi, S
        )
    ...
    def Tasks(self) -> None:
        ...
        # =====================Obstacle Avoidance===================================== #
        self.obstacle_pos = np.array([0, 0.6])
    ...
    def MPC(self) -> None:
        ...
        if self.node_id == 0:
            self.s = RobCont(
                omni=[np.array([0, 0, 0.05]) for _ in range(self.n_robots.omni)],
            )
        elif self.node_id == 1:
            self.s = RobCont(omni=[np.array([0, 1, -2.1]) for _ in range(self.n_robots.omni)])
        elif self.node_id == 2:
            self.s = RobCont(omni=[np.array([1, 1, 2.1]) for _ in range(self.n_robots.omni)])
        elif self.node_id == 3:
            self.s = RobCont(omni=[np.array([1, 0, 2]) for _ in range(self.n_robots.omni)])
        elif self.node_id == 4:
            self.s = RobCont(omni=[np.array([-0.5, -0.5, 0.25]) for _ in range(self.n_robots.omni)])
        elif self.node_id == 5:
            self.s = RobCont(omni=[np.array([4.48, 2.23, 3]) for _ in range(self.n_robots.omni)])
        elif self.node_id == 6:
            self.s = RobCont(
                omni=[np.array([-4.32, 2.51, 0.05]) for _ in range(self.n_robots.omni)]
            )
        elif self.node_id == 7:
    ...
    def update(self, round: str):
        ...
        if self.step < self.n_steps:
            ...
            self.u_star, self.y, A = self.hompc(
                copy.deepcopy(self.s_init.tolist()),
                rho_delta,
                null_method=st.null_method,
                A_neigh=NA_neigh,
            )
