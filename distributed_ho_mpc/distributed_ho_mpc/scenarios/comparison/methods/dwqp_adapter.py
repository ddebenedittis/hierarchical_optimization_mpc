"""Adapter that runs the radial-switching dWQP scenario on injected benchmark instances.

Wraps ``radial_switching_unicycle_wqp.network_simulation.main`` so it can be driven by the
...
def run_instance(instance: BenchmarkInstance, params: dict, out_dir: Path) -> RunResult:
    """Run the dWQP radial-switching scenario on one benchmark instance.

    Args:
        instance: Seeded benchmark instance (s_init, goals, config).
        params: Method params; may override 'comm_range' / 'limit_connection' / 'kappa'.
    ...
    """
    import distributed_ho_mpc.scenarios.radial_switching_unicycle_wqp.network_simulation as sim
    ...
    st.kappa = float(params.get('kappa', st.kappa))
