...
class Node:
    ...
    def __init__(
        ...
        if st.type == 'omni':
            self.s.omni, self.u.omni, self.s_kp1.omni = get_omnidirectional_model(5 * dt)
        elif st.type == 'uni':
    ...
    def Tasks(self) -> None:
        ...
        # =====================Obstacle Avoidance===================================== #
        self.obstacle_pos_1 = np.array([self.obstacles[0][0], self.obstacles[0][1]])
        self.obstacle_size_1 = self.obstacles[0][2] + 0.05
        self.task_obs_avoidance = [None, None, None]
        self.task_obs_avoidance[0] = [
            ca.vertcat(
                -((self.s.omni[0] - self.obstacle_pos_1[0]) ** 2)
                - (self.s.omni[1] - self.obstacle_pos_1[1]) ** 2
                + self.obstacle_size_1**2
            )
        ]

        self.obstacle_pos_2 = np.array([self.obstacles[1][0], self.obstacles[1][1]])
        self.obstacle_size_2 = self.obstacles[1][2] + 0.05
        self.task_obs_avoidance[1] = [
            ca.vertcat(
                -((self.s.omni[0] - self.obstacle_pos_2[0]) ** 2)
                - (self.s.omni[1] - self.obstacle_pos_2[1]) ** 2
                + self.obstacle_size_2**2
            )
        ]

        self.obstacle_pos_3 = np.array([self.obstacles[2][0], self.obstacles[2][1]])
        self.obstacle_size_3 = self.obstacles[2][2] + 0.05
        self.task_obs_avoidance[2] = [
            ca.vertcat(
                -((self.s.omni[0] - self.obstacle_pos_3[0]) ** 2)
                - (self.s.omni[1] - self.obstacle_pos_3[1]) ** 2
                + self.obstacle_size_3**2
            )
        ]
    ...
    def remove_connection(self, adjacency_vector: np.array, neigh_tasks: str, neigh_id: int):
        ...
        self.hompc.update_task(name='input_limits', prio=1, robot_index=[self.robot_idx])
        # self.hompc.update_task(name='input_smooth', prio=2, robot_index=[self.robot_idx])
        # self.hompc.update_task(name='obstacle_avoidance', prio=3, robot_index=[self.robot_idx])
        ...
        for n, task in enumerate(self.hompc._tasks):
            if task.type == TaskType.Bi:
                ...
                elif task.eq_coeff is not None:
                    # self.task_pos_coeff = [None for i in range(len(self.goals))]
                    # for i, g in enumerate(self.goals):
                    #     self.task_pos_coeff[i] = RobCont(
                    #         omni=[[g] for _ in range(self.n_robots.omni)],
