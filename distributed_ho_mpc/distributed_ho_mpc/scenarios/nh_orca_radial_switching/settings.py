...
epsilon = 0.1  # extra conservative (tracking-error) buffer this method holds beyond d_safe
# The enforced pairwise collision diameter is d_safe + 2*epsilon, not d_safe directly -- this
# baseline is deliberately more conservative than the shared measurement threshold, the same
# way a real NH-ORCA controller needs margin beyond the metric's collision distance to absorb
# tracking error. orca_radius is half that enforced diameter (each agent contributes half).
orca_radius = (d_safe + 2 * epsilon) / 2.0  # collision radius of each agent
