# ORCA Radial Switching scenario

## What this is

A **distributed, per-agent ORCA (Optimal Reciprocal Collision Avoidance)
controller**, run on the same radial-switching benchmark as
[`potential_field`](../potential_field/README.md) and
[`distributed_qp`](../distributed_qp/README.md): agents start on a circle and
...
```shell
python3 src/distributed_ho_mpc/distributed_ho_mpc/scenarios/orca_radial_switching/network_simulation.py
```
...
`distributed_qp`'s single per-agent QP safety filter, ORCA's collision
...
| `orca_radius` | Collision radius used to build each agent's velocity obstacle |
