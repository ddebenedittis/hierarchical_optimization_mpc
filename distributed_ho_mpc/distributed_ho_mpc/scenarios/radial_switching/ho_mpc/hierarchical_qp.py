...
class QPSolver(Enum):
    ...
    def get_enum(cls, solver):
        ...
        raise ValueError(
            f'The input solver is {solver}. Acceptable values are '
...
class HierarchicalQP:
    ...
    def solver(self, value):
        ...
        if isinstance(value, QPSolver):
            ...
            raise ValueError(
                f'The solver must be either a string or a QPSolver.'
    ...
    def _check_dimensions(A, b, C, d, we, wi, priorities):
        ...
        if priorities is not None:
            for i in range(n_tasks):
                if i not in priorities:
                    raise ValueError(f'priorities is ill formed: priorities = {priorities}')
        ...
        for i in range(n_tasks):
            if A[i].shape[0] != b[i].shape[0]:
                raise ValueError(f'At priority {i}, A and b have a different number of rows.')

            if C[i].shape[0] != d[i].shape[0]:
                raise ValueError(f'At priority {i}, C and d have a different number of rows.')

        for i in range(n_tasks):
            if A[i].shape[1] != nx:
                raise ValueError(f'At priority {i}, A has {A[i].shape[1]} columns instead of {nx}')

            if C[i].shape[1] != nx:
                raise ValueError(f'At priority {i}, C has {C[i].shape[1]} columns instead of {nx}')
    ...
    def _solve_qp(self, H, p, C, d, priority):
        ...
        if C.size == 0:
            if self._solver == QPSolver.reluqp:
                ...
                sol = solve_qp(
                    H, p, solver=self._solver.to_string(), **self._solver.get_solver_opts()
                )
        else:
            if self._solver == QPSolver.reluqp:
                ...
                model.setup(
                    my_from_numpy(H, device),
                    my_from_numpy(p, device),
                    my_from_numpy(C, device),
                    my_from_numpy(l, device),
                ...
                sol = solve_qp(
                    H, p, C, d, solver=self._solver.to_string(), **self._solver.get_solver_opts()
                )
            if sol is None:
    ...
    def rho_vector(self, rho, degree, n_c):
        ...
        s = np.concatenate(
            [[index[i : i + 3 : 1] for i in range(0 + (c * xdim), dim, x_i)] for c in range(n_c)]
        )
        reorder = np.concatenate([np.hstack(u), np.hstack(s)])
        return rho_vector[reorder]

    def _solve_weighted(self, A, b, C, d, we=None, wi=None, priorities=None) -> np.ndarray:
        n_tasks = len(A)
        ...
        n_eq = sum(
            [A[p].shape[0] if we[p] == np.inf and A[p] is not None else 0 for p in range(n_tasks)]
        )
        n_ie = sum([C[p].shape[0] if C[p] is not None else 0 for p in range(n_tasks)])
        n_slack = sum(
            [C[p].shape[0] if wi[p] != np.inf and C[p] is not None else 0 for p in range(n_tasks)]
        )
        ...
        for i in range(n_tasks):
            ...
            if wip is not np.inf:
                H_tot[
                    nx + i_slack : nx + i_slack + Cp.shape[0],
                    nx + i_slack : nx + i_slack + Cp.shape[0],
                ] = np.eye(Cp.shape[0]) * wip**2
            ...
            if Cp is not None:
                ...
                if wip is not np.inf:
                    C_tot[
                        ii : ii + Cp.shape[0], nx + i_slack : nx + i_slack + Cp.shape[0]
                    ] = -np.eye(Cp.shape[0])
                    i_slack += Cp.shape[0]
