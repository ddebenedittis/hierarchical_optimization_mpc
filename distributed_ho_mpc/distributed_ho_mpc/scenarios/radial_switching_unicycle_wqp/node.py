...
class Node:
    ...
    def MPC(self) -> None:
        ...
        for task in self.tasks:
            # Weight ladder for the weighted (non-hierarchical) solve: higher
            # priority (smaller prio number) gets a larger weight.
            prio_weight = st.kappa ** (st.n_priority - task['prio'])
            if task['name'] == 'input_limits':
                self.hompc.create_task(
                    ...
                    ineq_weight=np.inf,  # actuator limits stay a hard constraint
