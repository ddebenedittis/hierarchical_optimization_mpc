...
class HOMPC:
    ...
    def __init__(self, state: ca.SX, input: ca.SX, f: ca.SX, solver: QPSolver = QPSolver.quadprog):
        ...
        self._model = ca.Function('f', [state, input], [f], ['state', 'input'], ['state_kp1'])
    ...
    def _initialize(self, state_meas: np.ndarray, inputs: list[np.ndarray] = None):
        ...
        for i in range(self._n_control):
            self._state_bar[i + 1] = self._model(self._state_bar[i], self._input_bar[i]).full()
    ...
    def _linearize_model(
        ...
        A = subs([self._J_f_x], [self._state, self._input], [state_bar, input_bar])
        A = A  # + np.eye(*A.shape)

        B = subs([self._J_f_u], [self._state, self._input], [state_bar, input_bar])
    ...
    def __call__(
        self, state_meas: np.ndarray = None, inputs: list[np.ndarray] = None
    ) -> np.ndarray:
    ...
    def _get_n_x_opt(self) -> int:
        ...
        return self._n_control * self._n_input + (self._n_control + self._n_pred) * self._n_state
    ...
    def _get_idx_state_kp1(self, k) -> np.ndarray:
        ...
        return np.arange(n_c * n_i + k * n_s, n_c * n_i + (k + 1) * n_s)
