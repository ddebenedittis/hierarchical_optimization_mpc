...
class TasksCreatorHOMPCMultiRobot:
    ...
    def get_task_input_smooth(self):
        task_input_smooth = [
            ca.vertcat(self.u[0][0], -self.u[0][0], self.u[0][1], -self.u[0][1]),
            ca.vertcat(self.u[1][0], -self.u[1][0], self.u[1][1], -self.u[1][1]),
        ]
    ...
    def get_task_coverage(self, robot_idx: list[list[int]] = None):
        if robot_idx is None:
            towers = np.array(
                [e[0:2] for e in self.states_bar[0]] + [e[0:2] for e in self.states_bar[1]]
            )
            n_cov = self.n_robots
        else:
            towers = np.array(
        ...
        task_cov, task_cov_coeff = self.get_task_pos_ref(pos_ref, robot_idx)

        return task_cov, task_cov_coeff

    # ========================== Get_task_input_min ========================== #

    def get_task_input_min(
        self,
    ):
        return [
            ca.vertcat(
    ...
    def get_task_obs_avoidance(self, obstacle_pos: np.ndarray, threshold: float = 1.0):
        if len(obstacle_pos) != 2:
            raise ValueError('The obstacle position must be a vector of size 2.')

        return [
            ca.vertcat(
                -((self.s[0][0] - obstacle_pos[0]) ** 2)
                - (self.s[0][1] - obstacle_pos[1]) ** 2
                + threshold**2
            ),
            ca.vertcat(
                -((self.s[1][0] - obstacle_pos[0]) ** 2)
                - (self.s[1][1] - obstacle_pos[1]) ** 2
                + threshold**2
            ),
        ]
    ...
    def get_task_avoid_collision(self, threshold: float = 0.1):
        ...
        task_avoid_collision_coeff = [
            TaskBiCoeff(1, 0, 1, 1, 0, -(threshold**2)),
            TaskBiCoeff(1, 0, 1, 2, 0, -(threshold**2)),
            TaskBiCoeff(1, 0, 1, 3, 0, -(threshold**2)),
            TaskBiCoeff(1, 0, 1, 4, 0, -(threshold**2)),
            TaskBiCoeff(1, 1, 1, 2, 0, -(threshold**2)),
            TaskBiCoeff(1, 1, 1, 3, 0, -(threshold**2)),
            TaskBiCoeff(1, 1, 1, 4, 0, -(threshold**2)),
            TaskBiCoeff(1, 2, 1, 3, 0, -(threshold**2)),
            TaskBiCoeff(1, 2, 1, 4, 0, -(threshold**2)),
            TaskBiCoeff(1, 3, 1, 4, 0, -(threshold**2)),
        ]
