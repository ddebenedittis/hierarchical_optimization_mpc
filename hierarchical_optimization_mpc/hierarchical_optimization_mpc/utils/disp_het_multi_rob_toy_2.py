...
class Animation:
    ...
    def init(self):
        ...
        self.artists.omnidir = self.ax.scatter(
            [],
            [],
            s=25,
            c='C1',
        ...
        legend_elements.append(
            Line2D(
                ...
                linestyle='--',
                alpha=0.5,
                label='Trajectory -- weighted $\kappa=5$',
            )
        )
        legend_elements.append(
            Line2D(
                ...
                linestyle='--',
                alpha=0.5,
                label='Trajectory -- weighted $\kappa=100$',
            )
        )
    ...
    def update(self, frame):
        ...
        if sum(self.n_robots) > 1:
            self.artists.centroid.set_offsets(
                sum(
                    [
                        np.nan_to_num(np.mean(x[i][:, 0:2], axis=0)) * self.n_robots[i]
