...
class TestHOMPCMultiRobot:
    class HOMPCDesc:
        def __init__(
            self, states: list[ca.SX], inputs: list[ca.SX], fs: list[ca.SX],
            n_robots: list[int], hierarchical: bool, solver: QPSolver
        ) -> None:
            self.states = states
            self.inputs = inputs
            self.fs = fs
    ...
    def test_object_creation(self):
        try:
            ...
            n_robots = [2]
            hierarchical = True
            self.hompcs_desc.append(self.HOMPCDesc(
                s[0:1], u[0:1], s_kp1[0:1], n_robots, hierarchical, QPSolver.quadprog))
            
            for n_robots in [[2,0], [0,2], [0,2]]:
                hierarchical = True
                self.hompcs_desc.append(self.HOMPCDesc(
                    s, u, s_kp1, n_robots, hierarchical, QPSolver.quadprog))
            
        except Exception as e:
            assert False, f"HOMPCMultiRobot object creation with parameters " \
        ...
        for hompc_desc in self.hompcs_desc:
            try:
                ...
                task_coverage, task_coverage_coeff = tasks_creator.get_task_pos_ref(
                    [[np.random.rand(2) for n_j in range(hompc_desc.n_robots[c])] for c in range(len(hompc_desc.n_robots))]
                )
                
                task_charge, task_charge_coeff = tasks_creator.get_task_pos_ref(
                    [[np.array([19, 19]) for n_j in range(hompc_desc.n_robots[c])] for c in range(len(hompc_desc.n_robots))]
                )
                ...
                assert False, f"TasksCreatorHOMPCMultiRobot object creation with parameters " \
