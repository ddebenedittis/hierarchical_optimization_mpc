...
def main():
    ...
    s_kp1 = vertcat(s[0] + dt * u[0] * cos(s[2]), s[1] + dt * u[0] * sin(s[2]), s[2] + dt * u[1])
    ...
    task_input_limits = vertcat(u[0] - v_max, -u[0] + v_min, u[1] - omega_max, -u[1] + omega_min)
    task_input_limits_coeffs = [np.array([0, 0, 0, 0])]

    task_vel_reference = vertcat((s_kp1[0] - s[0]) / dt - 1, (s_kp1[1] - s[1]) / dt - 0)
